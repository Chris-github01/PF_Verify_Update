  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-400">Loading PDF vault...</div>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-lg border border-white/20 p-4">
          <div className="flex items-center gap-3">
            <FileText className="text-gray-400" size={24} />
            <div>
              <div className="text-sm text-gray-400">Total Line Items</div>
          ...
          <div className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-800/50 border border-slate-600 rounded-md text-white placeholder-gray-400 focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
              />
            </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {filteredQuotes.map((quote) => (
                <tr key={quote.quote_id} className="hover:bg-slate-800/30">
                  <td className="px-4 py-3">
                    <div>
                      <div className="font-medium text-white">{quote.supplier_name}</div>
                      {quote.quote_reference && (
                        <div className="text-sm text-gray-400">{quote.quote_reference}</div>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <div className="text-sm text-gray-300">{quote.project_name}</div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm font-medium text-white">{quote.items_count}</span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {quote.avg_confidence ? (
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-slate-700/50 rounded-full h-2 w-16">
                          <div
                    ...
                    <div className="flex gap-2">
                      {quote.file_url ? (
                        <button
                          ...
                          className="p-1.5 text-orange-500 hover:bg-orange-500/20 rounded transition-colors"
                          title="Download PDF"
                        >
                          <Download size={16} />
                        </button>
