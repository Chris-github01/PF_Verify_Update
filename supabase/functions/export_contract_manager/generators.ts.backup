...
export function generateJuniorPackHTML(
  ...
  // Helper function to parse detail strings into structured data
  function parseDetailString(detailStr: string) {
    // Format: "Description [Service: X | Type: Y | Material: Z | Qty: N unit]"
    const match = detailStr.match(/^(.+?)\s*\[Service:\s*(.+?)\s*\|\s*Type:\s*(.+?)\s*\|\s*Material:\s*(.+?)\s*\|\s*Qty:\s*(\d+(?:\.\d+)?)\s*(.+?)\]$/);
  ...
  // Group systems into pages - only create new page when there's too much content
  let systemsHTML = '';
  let currentPageSystems: any[] = [];
  let currentPageItemCount = 0;
  const MAX_ITEMS_PER_PAGE = 20; // Approx items that fit on one page

  scopeSystems.forEach((sys, index) => {
    const parsedDetails = sys.details && sys.details.length > 0
      ? sys.details.map((d: string) => parseDetailString(d))
      : [];

    // Check if adding this system would exceed page capacity
    const itemCount = parsedDetails.length;
    if (currentPageItemCount + itemCount > MAX_ITEMS_PER_PAGE && currentPageSystems.length > 0) {
      // Flush current page
      systemsHTML += generateJuniorPackPage(currentPageSystems, index === 0, organisationLogoUrl, parseDetailString);
      currentPageSystems = [];
      currentPageItemCount = 0;
    }

    currentPageSystems.push({ ...sys, parsedDetails });
    currentPageItemCount += itemCount;
  });

  // Flush remaining systems
  if (currentPageSystems.length > 0) {
    systemsHTML += generateJuniorPackPage(currentPageSystems, scopeSystems.length === currentPageSystems.length, organisationLogoUrl, parseDetailString);
  }

  function generateJuniorPackPage(systems: any[], isFirstPage: boolean, logoUrl?: string, parser?: Function) {
    const header = isFirstPage
      ? `<header>${generateLogoSection(logoUrl)}<div class="generated-by">Generated by <strong>VerifyTrade</strong></div></header><h2>Scope of Works Overview</h2>`
      : `<header>${generateLogoSection(logoUrl)}<div class="generated-by">Generated by <strong>VerifyTrade</strong></div></header>`;

    const systemsContent = systems.map(sys => {
      const detailsHTML = sys.parsedDetails && sys.parsedDetails.length > 0
        ? `
          <div style="background: #f9fafb; border-radius: 8px; padding: 16px; overflow-x: auto; margin-top: 12px;">
            <table style="width: 100%; border-collapse: collapse; background: white; border-radius: 8px; overflow: hidden;">
              ...
              <tbody>
                ${sys.parsedDetails.map((item: any, idx: number) => `
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .system-card { background: white; border: 2px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-bottom: 16px; page-break-inside: avoid; }
...
export function generateSeniorReportHTML(
  ...
  const scopeDetailsHTML = scopeSystems.map(sys => {
    const parsedDetails = sys.details.map((d: string) => parseDetailString(d));
