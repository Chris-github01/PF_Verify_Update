...
class ContractPackBuilder {
  ...
  buildSeniorManagementPack(data: ContractPackData): string {
    ...
    const projectArea = 5000;
    const pricePerM2 = data.financial.totalAmount / projectArea;
    const industryAverage = 95;
    const variance = ((pricePerM2 - industryAverage) / industryAverage) * 100;
    const varianceLabel = variance < 0 ? `${variance.toFixed(1)}%` : `+${variance.toFixed(1)}%`;
    const varianceColor = variance < 0 ? '#16a34a' : '#dc2626';

    // Generate cashflow projection
    const months = ['Month 1', 'Month 2', 'Month 3', 'Month 4', 'Month 5', 'Month 6'];
    const percentages = [5, 15, 25, 30, 20, 5];
    const maxAmount = data.financial.totalAmount * 0.30;
    ...
    return `
        ...
        <div style="display: grid; gap: 24px;">
          ...
          <div style="background: white; border: 2px solid #e5e7eb; border-radius: 12px; padding: 24px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 12px; font-size: 14px;">
              ...
              <div style="color: #6b7280; font-weight: 600;">Net Payable:</div>
              <div style="color: #16a34a; font-weight: 700;">${formatCurrency(netAmount)}</div>
      ...
      <!-- COMMERCIAL ANALYSIS & BENCHMARKING -->
      <div class="page">
        <header>
          ${this.layout.generateLogoSection(data.organisationLogoUrl)}
          <div class="generated-by">Generated by <strong>VerifyTrade</strong></div>
        ...
        <div style="background: white; border: 2px solid #e5e7eb; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
          <h3 style="color: ${VERIFYTRADE_ORANGE}; margin-bottom: 20px;">Market Benchmarking</h3>
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 20px;">
            <div style="text-align: center; padding: 20px; background: #f9fafb; border-radius: 8px;">
              <div style="font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px;">Project Rate ($/m²)</div>
              <div style="font-size: 28px; font-weight: 700; color: #111827;">${pricePerM2.toFixed(2)}</div>
            </div>
            <div style="text-align: center; padding: 20px; background: #f9fafb; border-radius: 8px;">
              <div style="font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px;">Industry Average ($/m²)</div>
              <div style="font-size: 28px; font-weight: 700; color: #111827;">${industryAverage.toFixed(2)}</div>
            </div>
            <div style="text-align: center; padding: 20px; background: #f9fafb; border-radius: 8px;">
              <div style="font-size: 11px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px;">Variance from Average</div>
              <div style="font-size: 28px; font-weight: 700; color: ${varianceColor};">${varianceLabel}</div>
            </div>
          </div>
          <div style="padding: 16px; background: ${variance < 0 ? '#dcfce7' : '#fef3c7'}; border-radius: 8px; border-left: 4px solid ${variance < 0 ? '#16a34a' : '#f59e0b'};">
            <p style="margin: 0; color: #374151; font-size: 13px;">
              ${variance < 0
                ? '✓ This quote is below market average, representing good value for this scope of work.'
                : '⚠ This quote is above market average. Consider value engineering or alternative solutions.'}
            </p>
          </div>
        </div>

        <div style="background: white; border: 2px solid #e5e7eb; border-radius: 12px; padding: 24px;">
          <h3 style="color: ${VERIFYTRADE_ORANGE}; margin-bottom: 20px;">Cashflow Forecast (6-Month Projection)</h3>
          <div style="display: flex; justify-content: space-around; align-items: flex-end; height: 180px; padding: 20px 0; border-bottom: 2px solid #e5e7eb;">
            ${months.map((month, idx) => {
              const amount = (data.financial.totalAmount * percentages[idx]) / 100;
              const height = (amount / maxAmount) * 100;
              return `
                <div style="flex: 1; display: flex; flex-direction: column; align-items: center; height: 100%;">
                  <div style="width: 60%; background: linear-gradient(to top, ${VERIFYTRADE_ORANGE}, ${VERIFYTRADE_ORANGE_LIGHT}); border-radius: 4px 4px 0 0; display: flex; align-items: flex-start; justify-content: center; padding-top: 8px; margin-bottom: 8px; height: ${height}%;">
                    <div style="font-size: 10px; font-weight: 700; color: #fff;">$${(amount / 1000).toFixed(0)}k</div>
                  </div>
                  <div style="font-size: 11px; color: #6b7280; text-align: center; margin-top: auto;">${month}</div>
                </div>
              `;
            }).join('')}
          </div>
          <div style="margin-top: 16px; padding: 12px; background: #f9fafb; border-radius: 8px; font-size: 12px; color: #6b7280;">
            <strong style="color: #111827;">Note:</strong> Projected cashflow follows typical S-curve distribution for passive fire protection installations.
          </div>
        </div>
        <footer>
          <div>© ${year} VerifyTrade. All rights reserved.</div>
      ...
      <div class="page">
        ...
        <div style="background: white; border: 2px solid #e5e7eb; border-radius: 12px; padding: 24px; overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
            <thead>
              <tr style="background: linear-gradient(135deg, #1f2937 0%, #374151 100%);">
                <th style="padding: 14px 10px; text-align: left; font-weight: 700; font-size: 11px; color: white; text-transform: uppercase; letter-spacing: 0.5px;">Risk Category</th>
                <th style="padding: 14px 10px; text-align: left; font-weight: 700; font-size: 11px; color: white; text-transform: uppercase; letter-spacing: 0.5px;">Description</th>
                <th style="padding: 14px 10px; text-align: left; font-weight: 700; font-size: 11px; color: white; text-transform: uppercase; letter-spacing: 0.5px;">Mitigation Strategy</th>
                <th style="padding: 14px 10px; text-align: center; font-weight: 700; font-size: 11px; color: white; text-transform: uppercase; letter-spacing: 0.5px;">Severity</th>
              </tr>
            </thead>
            <tbody>
              <tr style="background: #fee2e2;">
                <td style="padding: 12px 10px; color: #1f2937; font-weight: 600;">Quality & Compliance</td>
                <td style="padding: 12px 10px; color: #374151;">Non-compliant installations could fail fire engineer inspection</td>
                <td style="padding: 12px 10px; color: #374151;">Site supervision, QA checks, photo documentation, regular inspections</td>
                <td style="padding: 12px 10px; text-align: center;"><span style="background: #dc2626; color: white; padding: 4px 12px; border-radius: 4px; font-weight: 600; font-size: 10px;">HIGH</span></td>
              </tr>
              <tr style="background: #fef3c7;">
                <td style="padding: 12px 10px; color: #1f2937; font-weight: 600;">Programme Risk</td>
                <td style="padding: 12px 10px; color: #374151;">Delays in preceding trades may impact fire stopping schedule</td>
                <td style="padding: 12px 10px; color: #374151;">Weekly coordination meetings, early warning system, float buffer in programme</td>
                <td style="padding: 12px 10px; text-align: center;"><span style="background: #f59e0b; color: white; padding: 4px 12px; border-radius: 4px; font-weight: 600; font-size: 10px;">MEDIUM</span></td>
              </tr>
              <tr style="background: #fef3c7;">
                <td style="padding: 12px 10px; color: #1f2937; font-weight: 600;">Scope Clarity</td>
                <td style="padding: 12px 10px; color: #374151;">Potential for additional penetrations not shown on drawings</td>
                <td style="padding: 12px 10px; color: #374151;">Allowance included for additional works, variation process established</td>
                <td style="padding: 12px 10px; text-align: center;"><span style="background: #f59e0b; color: white; padding: 4px 12px; border-radius: 4px; font-weight: 600; font-size: 10px;">MEDIUM</span></td>
              </tr>
              <tr style="background: #fef3c7;">
                <td style="padding: 12px 10px; color: #1f2937; font-weight: 600;">Access & Coordination</td>
                <td style="padding: 12px 10px; color: #374151;">Access limitations due to concurrent trades</td>
                <td style="padding: 12px 10px; color: #374151;">Access equipment allowance, coordination with other trades, flexible resourcing</td>
                <td style="padding: 12px 10px; text-align: center;"><span style="background: #f59e0b; color: white; padding: 4px 12px; border-radius: 4px; font-weight: 600; font-size: 10px;">MEDIUM</span></td>
              </tr>
              <tr style="background: #dcfce7;">
                <td style="padding: 12px 10px; color: #1f2937; font-weight: 600;">Commercial</td>
                <td style="padding: 12px 10px; color: #374151;">Payment delays or retention release disputes</td>
                <td style="padding: 12px 10px; color: #374151;">Clear payment terms agreed, retention bond option, regular invoice tracking</td>
                <td style="padding: 12px 10px; text-align: center;"><span style="background: #16a34a; color: white; padding: 4px 12px; border-radius: 4px; font-weight: 600; font-size: 10px;">LOW</span></td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style="margin-top: 24px; padding: 20px; background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px;">
          <h3 style="color: #0c4a6e; margin: 0 0 12px 0; font-size: 16px;">Key Recommendations</h3>
          <ul style="margin: 0; padding-left: 20px; color: #0f172a; line-height: 1.8;">
            <li>Establish weekly coordination meetings with all trades</li>
            <li>Implement robust QA documentation process from day one</li>
            <li>Maintain photographic evidence of all penetrations</li>
            <li>Clarify variation process and pricing before commencement</li>
            <li>Ensure adequate float in programme for fire protection works</li>
          </ul>
        </div>
        <footer>
          <div>© ${year} VerifyTrade. All rights reserved.</div>
