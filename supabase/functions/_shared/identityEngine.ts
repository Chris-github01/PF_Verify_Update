// =============================================================================
// IDENTITY + AGGREGATION ENGINE  (Deno/Edge-Function copy)
...
export interface IdentityEngineResult {
  base: { items: IdentifiedItem[]; total: number };
  optional: { items: IdentifiedItem[]; total: number };
...
const UNIT_MAP: Record<string, string> = {
  no: 'ea', 'no.': 'ea', ea: 'ea', each: 'ea', item: 'ea', items: 'ea', nr: 'ea',
  m: 'lm', lm: 'lm', lin: 'lm', 'lin.m': 'lm',
  'm2': 'm2', sqm: 'm2', 'm²': 'm2',
  m3: 'm3', cbm: 'm3',
  'l/s': 'l/s', kg: 'kg', t: 't',
  hr: 'hr', hrs: 'hr', hour: 'hr', hours: 'hr',
  day: 'day', days: 'day', wk: 'wk', week: 'wk', weeks: 'wk',
  allow: 'allow', allowance: 'allow',
  sum: 'sum', ls: 'sum', 'l.s.': 'sum', 'l.s': 'sum',
};
...
export function buildIdentityKey(
  ...
  return `${block_id}|${system_type}|${signature}|${unit}|${unit_price.toFixed(4)}`;
}

// ---------------------------------------------------------------------------
// Step 7 — Filter non-scope items
// ---------------------------------------------------------------------------

const NON_SCOPE_RE: RegExp[] = [
  /^by\s+others\b/i,
...
function isNonScopeItem(item: RawParsedItem): boolean {
  ...
  if (!desc) return true;
  return NON_SCOPE_RE.some(re => re.test(desc));
}
...
// ---------------------------------------------------------------------------
// Step 6 — Aggregation Engine
// ---------------------------------------------------------------------------

export function aggregateItems(items: RawParsedItem[]): Map<string, IdentifiedItem> {
  ...
  for (const item of items) {
    ...
    if (rawSection) {
      const extracted = extractBlockId(rawSection);
      block_id = extracted !== 'UNKNOWN' ? extracted : rawSection.toUpperCase().slice(0, 20);
    }
    if (block_id === 'UNKNOWN') {
      const extracted = extractBlockId(desc);
      block_id = extracted !== 'UNKNOWN' ? extracted : lastBlockId;
    }
...
// ---------------------------------------------------------------------------
// Step 9 — Document Total Extraction
// ---------------------------------------------------------------------------
...
export function runIdentityEngine(
  ...
  const document_total = rawDocumentText ? extractDocumentGrandTotal(rawDocumentText) : null;
